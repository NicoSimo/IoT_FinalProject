...
public class CoapResourceGps2 extends CoapResource {

    private final static Logger logger = LoggerFactory.getLogger(CoapResourceGps2.class);
    private static final String OBJECT_TITLE = "GPS2";
    private static final Double SENSOR_VERSION = 0.1;

    private Gson gson;
    private GPSCar2 gps2;
    ...
    public CoapResourceGps2(String deviceId, String name, GPSCar2 gps2) {
        super(name);

        if (gps2 != null && deviceId != null) {
            this.gson = new Gson();
            ...
            getAttributes().addAttribute("rt", gps2.getType());
            getAttributes().addAttribute("if", CoreInterfaces.CORE_S.getValue());
            getAttributes().addAttribute("ct", Integer.toString(MediaTypeRegistry.APPLICATION_SENML_JSON));
            //getAttributes().addAttribute("ct", Integer.toString(MediaTypeRegistry.TEXT_PLAIN));

        } else {
            logger.error("Error occured in gps2");
        }

        this.gps2.addDataListener(new DataListener() {
            @Override
    ...
    public void handleGET(CoapExchange exchange) {
        ...
        try {
            ...
            exchange.setMaxAge(GPSCar2.UPDATE_PERIOD);
